...
import { ChainName, CONTRACTS, ethers_contracts, EVMChainName } from '@certusone/wormhole-sdk';
...
export function getEVMProvider(chain: EVMChainName): EVMProvider {
  if (!initializedEVMProviders[chain]) {
    if (chain === 'celo') {
      initializedEVMProviders[chain] = new CeloProvider(EVM_RPCS_BY_CHAIN[chain]);
    } else {
      initializedEVMProviders[chain] = new ethers.providers.JsonRpcBatchProvider(EVM_RPCS_BY_CHAIN[chain]);
    }
  }
...
export function createGetBlockByTagForEVM(chain: EVMChainName, finalizedBlockTag: 'finalized' | 'latest' = 'latest') {
  const provider = getEVMProvider(chain);
  return async (): GetFinalizedBlockNumberResult => safeGetBlockByTag(chain, finalizedBlockTag, provider);
}
...
export async function getMessagesForBlocksEVM(
  ...
  for (const log of logs) {
    ...
    const vaaKey = makeVaaKey(log.transactionHash, chain, emitter, sequence.toString());
