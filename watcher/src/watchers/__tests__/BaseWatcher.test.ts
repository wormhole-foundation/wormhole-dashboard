...
import { fixtureTest, withRpcReplay } from './rpcFixture.js';
...
test('getFinalizedBlockNumber', async () => {
  const watcher = new EVMWatcher('Mainnet', 'Base', 'finalized', 'vaa');
  const blockNumber = await watcher.getFinalizedBlockNumber();
  expect(blockNumber).toBeGreaterThan(initialBaseBlock);
});

// Blocks 1544175-1544185 are pruned on non-archival Base RPCs; this replays a
// recorded response to keep the block/timestamp decoding assertions deterministic.
fixtureTest('base-getMessagesForBlocks-1544175.json')('getMessagesForBlocks', () =>
  withRpcReplay('base-getMessagesForBlocks-1544175.json', async () => {
    const watcher = new EVMWatcher('Mainnet', 'Base', 'finalized', 'vaa');
