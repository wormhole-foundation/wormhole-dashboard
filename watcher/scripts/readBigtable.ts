...
(async () => {
  ...
  try {
    ...
    if (observedMessages[0][0]) {
      ...
      console.log(
        '   chain        ',
        parseInt(observedMessages[0][0]?.id.split('/')[0])
      );
      console.log(
        '   block        ',
        BigInt(MAX_UINT_64) -
          BigInt(observedMessages[0][0]?.id.split('/')[1] || 0)
      );
      console.log('   emitter      ', observedMessages[0][0]?.id.split('/')[2]);
      console.log(
        '   seq          ',
        parseInt(observedMessages[0][0]?.id.split('/')[3])
      );
      console.log(
        '   timestamp    ',
        observedMessages[0][0]?.data.info.timestamp[0].value
      );
      console.log(
        '   txHash       ',
        observedMessages[0][0]?.data.info.txHash[0].value
      );
      console.log(
        '   hasSignedVaa ',
        observedMessages[0][0]?.data.info.hasSignedVaa[0].value
      );
    }
    if (observedMessages[0][1]) {
      ...
      console.log(
        '   chain        ',
        parseInt(observedMessages[0][1]?.id.split('/')[0])
      );
      console.log(
        '   block        ',
        BigInt(MAX_UINT_64) -
          BigInt(observedMessages[0][1]?.id.split('/')[1] || 0)
      );
      console.log('   emitter      ', observedMessages[0][1]?.id.split('/')[2]);
      console.log(
        '   seq          ',
        parseInt(observedMessages[0][1]?.id.split('/')[3])
      );
      console.log(
        '   timestamp    ',
        observedMessages[0][1]?.data.info.timestamp[0].value
      );
      console.log(
        '   txHash       ',
        observedMessages[0][1]?.data.info.txHash[0].value
      );
      console.log(
        '   hasSignedVaa ',
        observedMessages[0][1]?.data.info.hasSignedVaa[0].value
      );
    }
  } catch (e) {
    console.error(e);
