...
export async function getVaasByTxHash(req: any, res: any) {
  ...
  try {
    ...
    const txHashRows = await vaasByTxHashTable.getRows({
      prefix: txHashKey,
      limit: 1,
    });
    ...
    const vaaKeys = JSON.parse(
      txHashRows[0][0].data.info.vaaKeys[0].value || []
    );
    if (vaaKeys.length === 0) {
      res.status(404);
    ...
    const signedVAAs = await signedVAAsTable.getRows({
      keys: vaaKeys,
      decode: false,
    });
    const result = vaaKeys.map((vaaKey: string) => {
      ...
      const vaaBytes = row
        ? row.data.info.bytes[0].value.toString('hex')
        : null;
      return {
        id: vaaKey,
        vaaBytes,
