      ...
      for (const log of logs) {
        ...
        if (log.topics[0] === TransferSentTopic) {
          ...
          if (lc) {
            ...
            this.logger.error(
              `TransferRedeemedTopic: Could not find lifecycle for digest ${digest}`
            );
            const lc: LifeCycle = {
              transferSentTxhash: '',
              transferBlockHeight: 0n,
              redeemedTxhash: txhash.startsWith('0x') ? txhash.slice(2) : txhash,
              redeemedBlockHeight: BigInt(blockNumber),
              sourceToken: '',
              nttTransferKey: '',
              vaaId: '',
              digest: digest,
...
function mergeLifecycles(lc: LifeCycle, existing: LifeCycle): LifeCycle {
  let merged: LifeCycle = {
    ...
    // Since isRelay is a boolean, we can't just look at the current value
    isRelay: lc.transferSentTxhash !== '' ? lc.isRelay : existing.isRelay,
    transferSentTxhash:
      lc.transferSentTxhash !== '' ? lc.transferSentTxhash : existing.transferSentTxhash,
    transferBlockHeight:
      lc.transferBlockHeight !== 0n ? lc.transferBlockHeight : existing.transferBlockHeight,
    redeemedTxhash: lc.redeemedTxhash !== '' ? lc.redeemedTxhash : existing.redeemedTxhash,
    redeemedBlockHeight:
      lc.redeemedBlockHeight !== 0n ? lc.redeemedBlockHeight : existing.redeemedBlockHeight,
    ...
    outboundTransferReleasableTime:
      lc.outboundTransferReleasableTime !== ''
        ? lc.outboundTransferReleasableTime
        : existing.outboundTransferReleasableTime,
  };
  return merged;
}
