...
export async function withRpcReplay(name: string, fn: () => Promise<void>): Promise<void> {
  if (RECORD) {
    ...
    (axios as any).post = async (url: string, body?: unknown, cfg?: unknown) => {
    ...
    let i = 0;
    // Explicitly save/restore axios.post instead of jest.spyOn().mockRestore(): under this
    // CJS jest setup, mockRestore() does not reliably restore the axios *module* mock, so the
    // replay stub would leak into subsequent tests (breaking their live RPC calls). Direct
